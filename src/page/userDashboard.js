...
const UserDashboard = () => {
  ...
  // 2. 👇 Extract jwtToken and userName from Context
  const { jwtToken, userName } = useAppId();

  useEffect(() => {
    // 3. 👇 Use token for expenses request
    if (!jwtToken) return;
  ...
  }, [jwtToken]) // Added dependency

  useEffect(() => {
    // 4. 👇 Use token for incomes request
    if (!jwtToken) return;
            ...
            <tbody>
              {expenses.map((item, idx) => (
                <tr key={idx}>
                  ...
                  <td><span className={`pill-status ₹{item.status}`}>{item.UserName}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
